...
protocol TaskProgressPresenter where Self : UIViewController {
    func startAnimatingIndicator(config: UIActivityIndicatorViewConfiguration)
    func stopAnimatingIndicator()
}

extension TaskProgressPresenter {
    
    func startAnimatingIndicator(config: UIActivityIndicatorViewConfiguration = .init()) {
        let indicator = activityIndicator(config: config)
        indicator.startAnimating()
    }
    
    func stopAnimatingIndicator() {
        guard let indicator = self.view.subviews.last as? UIActivityIndicatorView else { return }
        indicator.removeFromSuperview()
    }
    
    private func activityIndicator(config: UIActivityIndicatorViewConfiguration = .init()) -> UIActivityIndicatorView {
        let indicator = UIActivityIndicatorView(style: config.style)
        self.view.addSubview(indicator)
        indicator.hidesWhenStopped = true
        switch config.center {
        case .superviewCenter:
...
struct UIActivityIndicatorViewConfiguration {
    var style: UIActivityIndicatorView.Style = .large
    var center: Center = .superviewCenter
...
extension UIViewController : TaskProgressPresenter { }
